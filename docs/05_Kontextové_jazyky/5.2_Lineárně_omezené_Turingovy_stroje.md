...
<a id="def-ts"></a>
!!! Definition "Definice (Turingův stroj)"
...
<a id="def-dts"></a>
!!! Definition "Definice (Deterministický Turingův stroj)"
    ...
    **Deterministický Turingův stroj** (DTS) pracuje s následující definicí přechodové funkce $\delta$:
    ...
    ![Turingův stroj](../assets/05/turinguv_stroj_white.png#only-dark){ align=center width="75%" }
    ![Turingův stroj](../assets/05/turinguv_stroj_black.png#only-light){ align=center width="75%" }
...
<a id="def-nts"></a>
!!! Definition "Definice (Nedeterministický Turingův stroj)"
...
*   **Ohodnocený orientovaný graf:** Vrcholy grafu reprezentují stavy. Přechodovou funkci znázorňujeme orientovanými hranami mezi stavy a ohodnocujeme je čtenými symboly, symboly k zápisu a pohybem hlavy.
    ...
    ![Stavový diagram](../assets/05/stavovy_diagram_white.png#only-dark){ align=center width="50%" }
    ![Stavový diagram](../assets/05/stavovy_diagram_black.png#only-light){ align=center width="50%" }

<a id="def-lba"></a>
!!! Definition "Definice (Lineárně omezený Turingův stroj)"
...
!!! Implication "Kdy Turingův stroj (ne)přijme vstupní řetězec?"

    **Deterministický Turingův stroj (DTS):**
    
    *   **Přijme:** Přejde do koncového stavu a páska je po ukončení výpočtu prázdná (vyplněna symboly $B$).
    *   **Nepřijme:**
        *   Dojde k chybě (nedefinovaná přechodová funkce).
        *   Výpočet neskončí (TS se zacyklí).
        *   Přejde do koncového stavu, ale páska není prázdná.

    **Nedeterministický Turingův stroj (NTS):**
    
    *   **Přijme:** Existuje alespoň jedna posloupnost přechodů, kdy TS přejde do koncového stavu a páska je prázdná.
    *   **Nepřijme:** Neexistuje posloupnost přechodů vedoucí k přijetí (všechny skončí chybou, zacyklením nebo neprázdnou páskou).